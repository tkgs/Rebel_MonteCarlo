...
from Rebel_MonteCarlo.utils import montecarlo, calc_pmt, montecarlo_bool, calc_payback, calc_duration, create_figure
...
def montecarlo_simulation(prazo, juros, divida, n_sim=1000, curva_default=[0]):
    ...
    # curva_default = [i+1 for i in range(1,37)]
    ...
    # print(dividas)
    # print('Avg', s.mean(prazos))
    # print('Max', max(prazos))
    # print('Min', min(prazos))
    # print('Std', s.stdev(prazos))
    #
    # print('Avg Juros', s.mean(juros))
    # print('Max', max(juros))
    # print('Min', min(juros))
    # print('Std', s.stdev(juros))
    #
    # print('Avg Divida', s.mean(dividas))
    # print('Max', max(dividas))
    # print('Min', min(dividas))
    # print('Std', s.stdev(dividas))
    ...
    for i in range(n_sim):
        ...
        pmts[prazo_int:] = 0
        # print(cashflows)
        # Insere o valor da divida na posicao 0
        pmts = np.insert(pmts, 0, -dividas[i])
        ...
        for p in range(1, prazo_int):
            pmts[p] = pmts[p] * montecarlo_bool(curva_default[p-1]/100)
        cashflows_mc += pmts

    # print('Dividas:', dividas)
    # print('Juros:', juros)
    # print('Prazos:', prazos)
    # print('PMT:', pmt)
    # print(irr)
    # print('Cashflows:', cashflows_total)
    irr_nodefault = npf.irr(cashflows_total)
    irr_mc = npf.irr(cashflows_mc)

        # figure.savefig('test.png')

    # fig = plt.subplot(3, 2, 6)
    # fig.text(0.5, 0.75, 'IRR: '+str(round(irr*100,2))+'% a.m.', fontsize=14, horizontalalignment='center', verticalalignment='center')
    # fig.text(0.5, 0.60, 'Payback: {:.1f} meses'.format(payback(cashflows_total)), fontsize=14, horizontalalignment='center', verticalalignment='center')
    # fig.text(0.5, 0.45, 'Total dívidas: R$ {}'.format(number_format(sum(dividas))), fontsize=14, horizontalalignment='center', verticalalignment='center')
    # fig.text(0.5, 0.30, 'Net cashflows: R$ {}'.format(number_format(cashflows_total.sum())), fontsize=14, horizontalalignment='center', verticalalignment='center')
    #
    # plt.axis('off')
    # # plt.text.set_clip_on(False) # I added this due to the answer from tcaswell
    # plt.tight_layout()
    # plt.show()

    total_dividas = sum(dividas)
    net_cashflows = cashflows_mc.sum()
    payback = calc_payback(cashflows_mc)
    duration_mc = calc_duration(cashflows_mc, irr_mc)
    duration_nodefault = calc_duration(cashflows_total, irr_nodefault)
    # Calcula dv01
    dv01 = (abs(npf.npv(irr_mc + 0.0001, cashflows_mc)) + abs(npf.npv(irr_mc - 0.0001, cashflows_mc))) / 2
    ...
    duration = [duration_mc, duration_nodefault]

    # Salva os gráficos no arquivo static/output.png
